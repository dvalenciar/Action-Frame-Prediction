...
Description: *** Evaluate Frame Prediction of Car Racing DataSet ***
...
from Intro_VAE_Network import IntroVAE
...
def transport_shape(y_prediction, t_target):
    # reshape from (32,3,64,64) to (32,64,64,3)
...
def calculate_mse_ssim_psnr_batch(y_prediction, t_target):
    # calculate mse, psnr and ssim of each bath of images
...
def plot_samples(y_prediction, t_target, mode, num_img):
    ...
    y_prediction, t_target = transport_shape(y_prediction, t_target)
    choose_images = [0, 34, 68, 212, 224, 194, 243]
...
def evaluate_model_sequence(model, data_input_test, data_target_test, data_actions_test, long_sequence):
    ...
    x_inp = x_inp[45:]  # to start from frame 45
    a_inp = a_inp[45:]
    y_tar = y_tar[45:]

    with torch.no_grad():
        image_input_true  = x_inp[0].unsqueeze(0)  # reshape from [3,64,64] to [1,3,64,4]
        predictions = []
...
def evaluation_mode():
    ...
    model_saved_path = './Model_Saved/model_intro_vae_completed_after_3000_epochs.pt'
    model.load_state_dict(torch.load(model_saved_path, map_location=torch.device('cpu')))

    #model_saved_path = './Model_Saved/checkpoint_2000_epochs.pt'
